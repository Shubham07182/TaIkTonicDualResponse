...
def answer_with_consistency_check(user_query):
    realtime_keywords = ["temperature", "weather", "forecast", "news", "stock", "price", "live", "current"]
    fact_keywords = ["president", "prime minister", "ceo", "current leader", "today", "as of now"]

    # ✅ Always use web search for real-time or factual queries
    if any(word in user_query.lower() for word in realtime_keywords + fact_keywords):
        return web_search_with_google_custom_search(user_query)
    
    # Normal LLM with consistency check
    answer1 = call_groq_model(user_query)

    reflection_prompt = f"Check this answer for correctness, consistency, and completeness. Reply with 'YES' if consistent, otherwise explain briefly:\n\n{answer1}"
    reflection = call_groq_model(reflection_prompt)

    if "yes" in reflection.lower():
        return answer1  
    answer2 = call_groq_model(f"Answer this differently but accurately: {user_query}")

    similarity = SequenceMatcher(None, answer1.lower(), answer2.lower()).ratio()

    if similarity > 0.65:  
        return f"{answer1}\n\n(Verified for consistency ✅ — similarity {similarity:.2f})"
    else:
        return f"Answer 1:\n{answer1}\n\nAnswer 2:\n{answer2}\n\n(⚠️ Inconsistent responses detected — similarity {similarity:.2f})"
...
def web_search_with_google_custom_search(query):
    try:
        ...
        for item in data["items"][:5]:
            ...
            link = item.get("link", "")
            top_snippets.append(f"{title}: {snippet}\n(Source: {link})")

        summary_prompt = f"""
        Summarize the following search results in 4-5 sentences.
        Make the answer clear and concise for the user:
...
if user_input:
    ...
    try:
        if is_csv(user_input):
            ...
            llm_resp = answer_with_consistency_check(input_text)
            triggers = ["I'm just an ","Since","large","can't","not available","unable","sorry","hasn't","wasn't","As of","has not","clarify","As of"]
